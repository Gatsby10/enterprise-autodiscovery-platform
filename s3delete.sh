...
VERSIONS=$(aws s3api list-object-versions --bucket "pet-adoption-state-bucket-one" --region "eu-west-3" \
...
if [[ "$VERSIONS" != "null" && "$VERSIONS" != "[]" ]]; then
  echo "$VERSIONS" | jq -c '.[]' | while read -r obj; do
    ...
    aws s3api delete-object --bucket "pet-adoption-state-bucket-one" --key "$KEY" --version-id "$VID" --region "eu-west-3" > /dev/null
  done
fi
...
MARKERS=$(aws s3api list-object-versions --bucket "pet-adoption-state-bucket-one" --region "eu-west-3" \
...
aws dynamodb delete-table --table-name "pet-adoption-state-bucket-one-dynamodb-lock" --region "eu-west-3" > /dev/null
echo "✔ DynamoDB lock table deleted: pet-adoption-state-bucket-one-dynamodb-lock"
...
echo "✔ Bucket deleted: pet-adoption-state-bucket-one"
